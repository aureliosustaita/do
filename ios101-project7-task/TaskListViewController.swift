...
//        let cell = tableView.dequeueReusableCell(withIdentifier: "TaskCell", for: indexPath) as! TaskCell
//        let task = tasks[indexPath.row]
//        cell.configure(with: task, onTaskTitleUpdated: { [weak self] updatedTask in
...
//    }
//
//    // "+" button tapped: add a new task inline
//    @IBAction func didTapNewTaskButton(_ sender: Any) {
//        let newTask = Task(title: "New Task")
//        tasks.append(newTask)
//        Task.save(tasks)
...
//    }
//
//    // Refresh tasks from storage, sort, and reload table
//    private func refreshTasks() {
//        var tasks = Task.getTasks()
//
//        tasks.sort {
...
//extension TaskListViewController: UITableViewDelegate {
//
//    // No segue, only inline editing, so no action needed
//    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
//        tableView.deselectRow(at: indexPath, animated: false)
//    }
//}
...
//            detailVC.onTaskDeleted = { [weak self] taskToDelete in
//                guard let self = self else { return }
...
extension TaskListViewController: UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        // Delay to check for double tap (editing mode)
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) { [weak self] in
            guard let self = self else { return }

            // Check if the tapped cell is not in editing mode
            if let cell = tableView.cellForRow(at: indexPath) as? TaskCell, !cell.titleTextView.isFirstResponder {
                // Proceed to open detail view
...
//
//    @IBAction func didTapNewTaskButton(_ sender: Any) {
//        let newTask = Task(title: "New Task", createdDate: Date())
//        tasks.append(newTask)
//        Task.save(tasks)
//        tableView.insertRows(at: [IndexPath(row: tasks.count - 1, section: 0)], with: .automatic)
//        emptyStateLabel.isHidden = true
...
//        var tasks = Task.getTasks()
//
//        tasks = tasks.filter { !$0.isHiddenFromTaskList }
//
//        tasks.sort {
//            if $0.isComplete && $1.isComplete {
//                return true
...
//    }
//
//    // MARK: - Delete Logic (Just Hide)
//    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
//        if editingStyle == .delete {
//            tasks[indexPath.row].isHiddenFromTaskList = true
//            Task.save(tasks)
//            refreshTasks()
//        }
//    }
